...
NATURAL = iterate(1, lambda x:x+1)

class StreamTest(unittest.TestCase):
    def setUp(self):
        self.some_nums = genstream_from([3,6,9])
        self.some_chars = genstream_from(list("abc"))
        self.some_primes = genstream_from([7,11,19])
    ...
    def test_primes(self):
        all_primes = stream_filter(NATURAL, is_prime)
    ...
    def test_genstream_it(self):
        ns = (i for i in [1,2,3])
        s = genstream_it(ns)
        self.assertEqual(stream_sum(s), 6)

    def test_genstream_it_pitfall(self):
        ns = (i for i in [1,2,3])
        s = genstream_it(ns)
        self.assertEqual(stream_sum(s), 6)
    ...
    def test_chain_simple(self):
        chained = genstream_chain(self.some_nums)
        self.assertStreamIs(chained, [3,6,9])

    def test_chain_complex(self):
        chained = genstream_chain(self.some_nums, self.some_chars, self.some_primes)
        self.assertStreamIs(chained, [3,6,9,'a','b','c',7,11,19])

    def test_chain_two(self):
        chained = genstream_chain_two(self.some_nums, self.some_chars)
        self.assertStreamIs(chained, [3,6,9,'a','b','c'])

    def test_chain_two_twice(self):
        c2 = genstream_chain_two(self.some_nums, self.some_chars)
        c3 = genstream_chain_two(c2, self.some_primes)
