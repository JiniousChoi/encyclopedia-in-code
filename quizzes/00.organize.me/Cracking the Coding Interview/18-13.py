...
#    word_length = len(words[0])
#    for chosens in permutations(words, 2*word_length):
#        print(chosens)
#        if is_correct(chosens):
#            return chosens
#    else:
#        return None
...
def make_words_matrix(words):
    ...
    from itertools import permutations
    for pivot1, pivot2 in permutations(words, 2):
        #print(pivot1, pivot2)
        for chosens in get_candidates(pivot1, pivot2, words):
            #print(chosens)
            if is_correct(chosens, words):
                assert pivot1 not in chosens
                assert pivot2 not in chosens
                return chosens
    else:
        return None

def get_candidates(pivot1, pivot2, words):
    nested_list = []
    for c1, c2 in zip(pivot1, pivot2):
        nested_list.append(get_word_list_that_startswith(c1, c2, words, excludes=[pivot1, pivot2]))
...
def get_word_list_that_startswith(c1, c2, words, excludes=[]):
    assert isinstance(excludes, list)
    result = []
    for word in words:
        if word not in excludes and word.startswith(c1) and word.endswith(c2):
            result.append(word)
    return result
